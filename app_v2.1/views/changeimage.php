  ...
  <body>
    ...
    <div class="container">
		...
		<div class="starter-template">
			<h1>Къам аивын</h1>
			<p>Ацы сыфыл ис ныхасы къам баивын</p>
			<hr>
			...
			<div>
			<? if ($finded['image'] != ''): ?>
				<img src='/u/<?=$finded['image']?>' alt='<?=$finded['word']?>' style='max-width: 100%; width:300px;' />
			<? endif ?>
			</div>
			...
			<form method="post" enctype="multipart/form-data" action="/admin/uploadImage">
				...
				<div class="form-group">
					<label>Къам равзарын</label>
					<input type='file' name='kam' />
				</div>
				
				<button type='submit' name='submit' class='btn btn-success btn-lg' id='save'>Къам баивын</button>
			</form>
			
			<? if ($finded['image'] != ''): ?>
			<br>
			<br>
			<form method="post" action="/admin/removeImage">
				<input type='hidden' id='nyhas_id' name='nyhas_id' value='<?=$finded['id']?>'>
				<input type='hidden' name='nyhas_word' value='<?=$finded['word']?>'>
				<button type='submit' name='submit' class='btn btn-danger btn-lg' id='save'>Къам айсын</button>
			</form>
			<? endif ?>
		</div>
    </div>
